...
- [26. 「記す（しるす）」與「印（しるし）」的字源淵源](#26-記すしるす與印しるしの字源淵源)
...
### 26. 「記す（しるす）」與「印（しるし）」的字源淵源 (🟡)

這兩個詞的日文漢字分別寫作「記」與「印」，但它們的讀音高度重合（`しるす` 與 `しるし`），這是因為它們在日語字源上是**「動詞」與「名詞形」的變身關係**：

#### 💡 語意演變：從「留下痕跡」到「記錄」與「記號」
在古代日語中，這兩個詞的共同起點是**「在某物上刻下、留下明確的標記或痕跡，使人看見並記住」**。
*   **記す（しるす）【動詞】**：
    *   *字源*：主動去「留下標記、刻下痕跡」➔ 延伸為在紙上**「書寫、記錄、記下」**，或者在心中**「銘記」**。
    *   *例句*：`心に記す` (銘記在心) / `歴史に記された出来事` (記錄在歷史中的事件)。
*   **印（しるし）【名詞】**：
    *   *字源*：動詞 `しるす` 的連用名詞形，指「被刻下、留下來的那個標記本身」 ➔ 延伸為現代日語的**「標記、記號、特徵、象徵」**，或者**「印記、證明」**。
    *   *例句*：`正しい答えに印をつける` (在正確答案上做記號) / `和平の印` (和平的象徵) / `感謝の印` (感謝的證明/謝意)。
